...
class CaBgpSecServiceTest {
    ...
    class CreateRouterKey {
        ...
        void shouldCreateRouterKey() throws Exception {
            ...
            ArgumentCaptor<AddBgpSecConfigurationCommand> captor = ArgumentCaptor.forClass(AddBgpSecConfigurationCommand.class);
            verify(commandService).execute(captor.capture());
            var command = captor.getValue();
            assertThat(command).isEqualTo(
                    new AddBgpSecConfigurationCommand(certificateAuthorityData.getVersionedId(), Asn.parse("AS10"), new RouterId(10L), CSR)
            );
        }
    ...
    class RouterKey {
        ...
        final BgpSecConfigurationData certificate = new BgpSecConfigurationData(
                routerKey.id(),
                routerKey.asn(),
                routerKey.routerId(),
                routerKey.csr(),
                routerKey.keyIdentifier(),
                null,
                null
        );
        ...
        class Get {
            ...
            @Test
            void shouldSerializeValidityPeriod() throws Exception {
                var notValidBefore = new DateTime(2024, 1, 15, 10, 30, 0, DateTimeZone.UTC);
                var notValidAfter = new DateTime(2025, 1, 15, 10, 30, 0, DateTimeZone.UTC);
                var certificateWithValidity = new BgpSecConfigurationData(
                        routerKey.id(),
                        routerKey.asn(),
                        routerKey.routerId(),
                        routerKey.csr(),
                ...
                assertThat(OffsetDateTime.parse(json.get("notValidBefore").asText()).toInstant())
                        .isEqualTo(Instant.ofEpochMilli(notValidBefore.getMillis()));
                assertThat(OffsetDateTime.parse(json.get("notValidAfter").asText()).toInstant())
                        .isEqualTo(Instant.ofEpochMilli(notValidAfter.getMillis()));
            }
        }
