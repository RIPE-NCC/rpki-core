...
public class PublisherRepositoriesServiceTest {
    ...
    @Test
    public void should_list_non_hosted_publisher_repositories() throws Exception {
        ...
        when(certificateAuthorityViewService.findNonHostedPublisherRepositories(NON_HOSTED_CA_DATA.getName()))
            .thenReturn(Collections.singletonMap(publisherHandle, new RepositoryResponse(
                Optional.empty(),
        ...
        assertThat(response.getContentAsString()).contains("\"publisherHandle\":\"" + publisherHandle + "\"");
    }
    ...
    @Test
    public void should_download_non_hosted_publisher_repository_response() throws Exception {
        UUID publisherHandle = UUID.randomUUID();
        when(certificateAuthorityViewService.findCertificateAuthorityByName(NON_HOSTED_CA_DATA.getName()))
                .thenReturn(NON_HOSTED_CA_DATA);
        when(certificateAuthorityViewService.findNonHostedPublisherRepositories(NON_HOSTED_CA_DATA.getName()))
        ...
        final MockHttpServletResponse response = result.getResponse();
        assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());
        assertThat(response.getContentAsString()).contains("publisher_handle=\"" + publisherHandle + "\"");
    }
