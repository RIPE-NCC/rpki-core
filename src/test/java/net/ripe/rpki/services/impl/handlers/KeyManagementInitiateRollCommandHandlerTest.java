...
public class KeyManagementInitiateRollCommandHandlerTest {
    ...
    private KeyPairEntity keyPairEntity;
    ...
    public void setUp() {
        ...
        subject = new KeyManagementInitiateRollCommandHandler(certificateAuthorityRepository, certificateRequestCreationService, resourceCertificateRepository, manifestPublicationService);
        keyPairEntity = mock(KeyPairEntity.class);
    ...
    public void shouldThrowExceptionIfCommandHadNoEffect() {
        ...
        when(certificateRequestCreationService.initiateKeyRoll(memberCA, THRESHOLD)).thenReturn(Optional.empty());
    ...
    public void shouldDelegateRequestsToAllResourcesCa() {
        ...
        when(certificateRequestCreationService.initiateKeyRoll(allResourcesCA, THRESHOLD)).thenReturn(Optional.of(request));
    ...
    public void shouldDelegateRequestsToProductionCa() {
        ...
        when(certificateRequestCreationService.initiateKeyRoll(productionCA, THRESHOLD)).thenReturn(Optional.of(request));
        ...
        when(productionCA.findKeyPairByPublicKey(request.getSubjectPublicKey())).thenReturn(Optional.of(keyPairEntity));
        ...
        verify(manifestPublicationService).publishRpkiObjectsIfNeeded(keyPairEntity);
    }

    @Test
    public void shouldDelegateRequestsToMemberCa() {
        when(certificateAuthorityRepository.findManagedCa(CA_ID)).thenReturn(memberCA);
        when(certificateRequestCreationService.initiateKeyRoll(memberCA, THRESHOLD)).thenReturn(Optional.of(request));
        ...
        when(memberCA.findKeyPairByPublicKey(request.getSubjectPublicKey())).thenReturn(Optional.of(keyPairEntity));
